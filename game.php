...
if( isset( $_SESSION['current_game_id'] ) && $_SESSION['current_game_id'] != 0 ){
	...
	$('#char_multiremove').click( function(){

		var r = confirm("Really delete characters?");

		if (r == true) {

			var char_ids = new Array();

			$('.checkboxform input:checked').each( function(){

				char_ids.push( $(this).data("char-id") );

			});

			$.get( 'setdata.php', { 'page':'char_multiremove', char_ids : char_ids }, function(z){
			});
	...
	$.getJSON("getdata.php", {
		...
		.done(function(json) {
			...
			if( game.fields.length > 0 ){
				...
				game.fields.forEach( function( field, index ){
					...
					if( field.field_owner_id == 0 ){
						...
						if( controller_user_id == <?=$_SESSION['user_id'];?> ){
							...
							if( field.field_cost_paid && field.field_cost_paid === false && field.cost.length > 0 ){
								...
								costpaybutton = '<a tabindex="-1" role="button" class="btn alert alert-danger btn-outline-light" style="height:30px;margin:1px;" onclick="paycost('+field.creation_date+')">Pay cost</a>';
					...
					if( field.field_status && field.field_status.length > 0 ){
						...
						field.field_status.forEach( function( state_id, i ){
							...
							for( i=0,j=lib.states.length;i<j;i++ ){
								...
								if( lib.states[i].state_id == state_id ){
									...
									for( k=0,l=lib.states[i].vars.length;k<l;k++ ){
										...
										add_li += '<li class=\'text-light\'>'+lib.states[i].vars[k].path+' '+lib.states[i].vars[k].modifier+'</li>';
							...
							states += 	'<a tabindex="-1" role="button" class="btn btn-dark btn-outline-light" style="height:30px;margin:1px;"'+
							' data-toggle="popover" title="'+state_name+'" data-placement="bottom" data-content="'+content+'" tabindex="-1">'+
								state_name+
							'</a>';
					...
					if( field.field_events && field.field_events.length > 0 ){
						...
						field.field_events.forEach( function( event, i ){
							...
							events += 	'<a tabindex="-1" role="button" class="btn btn-dark btn-outline-light" style="height:30px;margin:1px;"'+
									' data-toggle="popover" title="'+event.event_time+'" data-placement="bottom" data-content="'+content+'" data-trigger="focus">'+
					...
					field.cost.forEach( function( cost, i ){
						...
						cost_str += '<a tabindex="-1" role="button" class="btn btn-dark btn-outline-light" style="height:30px;margin:1px;">'+
				...
				$('[data-toggle="popover"]').popover({html:true,boundary:'viewport'});
...
$navbar .= '
	...
	<div id="special_buttons" style="width:250px;">
		<button type="button" class="form-control btn btn-dark" id="char_multiremove" style="float:left;width:75px;">'.$l[$lang]['GAME_CHARMULTIDELETE'].'</button>
