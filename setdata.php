...
class game{
	...
	public function setstate( $state_name, $state_id, $rounds, $user_id, $username, $state_vars = array(), $tank = array() ){
		...
		$this->char->states[] = (object) array( "name" => $state_name, "id" => $state_id, "rounds" => $rounds, "caster" => $user_id, "caster_username" => $username, "vars" => $state_vars, "tank" => $tank );
	...
	*/
	...
	public function calculate_damage( $action, $damage, $tier_lvl_index, $add_tokens, $options ){
		...
		foreach( $damage AS $dmg ){
			...
			foreach( $action->attributes AS $attr ){
				...
				if( $this->char->attributes->{$attr}->mod > $attribut ){
					
					$attribut = $this->char->attributes->{$attr}->mod;
					
				}
				
			}
			
			$dmg_formula = str_replace( 'attribute', $attribut, $dmg_formula );
	...
	public function execute_effects_summon_char( $char_id, $target_char_id, $action, $effects_summon_char ){
		...
		foreach( $effects_summon_char AS $effect ){
			...
			$this->add_char_to_game( $this->get_user_id_by_name2( $effect->char_name ), $this->char->leftright );
...
array_multisort( $matches[2], SORT_ASC, SORT_NATURAL );
				...
				foreach( $field_target_ids AS $target_id ){
					...
					if( !in_array( $target_id, $field->field_target_ids ) ){
						...
						foreach( $field->field_status AS $state_id ){
							...
							$this->setstate( '', $state_id, 999, $field_owner_id, $username );
			...
			if( $field->creation_date == $field_id ){
				...
				if( !in_array( $state_id, $field->status ) ){
					...
					foreach( $field->field_target_ids AS $char_id ){
						...
						$this->char( $char_id );
						$this->setstate( '', $state_id, 999, $field->field_owner_id, $username );
	...
	public function load_field( $field_id, $field_owner_id = 0, $field_target_ids = array() ){
		...
		if( $field_owner_id != 0 ){
			...
			$add_field->field_owner_id = $field_owner_id[0];
		...
		if( !empty( $field_target_ids ) ){
			...
			foreach( $field_target_ids as $target ){
				...
				foreach( $add_field->field_status as $state_id ){
					...
					$this->char( $target );
					$this->setstate( '', $state_id, 99, $add_field->field_owner_id, $this->char_names[$add_field->field_owner_id] );
