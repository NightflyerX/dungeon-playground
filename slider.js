    ...
    $.fn.freshslider = function(options){
        ...
        var updateCarets = function(){
            if(text){
	            ...
	            var skill_pot = [ "X", "N", "E", "M", "GM" ];
	            
                caretRight.text( skill_pot[(round(values[1] * gap) + min).toFixed(countPoint)] + unit);
                if(!isSingle){
                    caretLeft.text( skill_pot[(round(values[0] * gap) + min).toFixed(countPoint)] + unit);
                }
            }
            ...
            if(updateCallback){
                if(isSingle){
                    ...
                    updateCallback(round(values[0] * gap) + min, round(values[1] * gap ) + min, me );
                }
            }
