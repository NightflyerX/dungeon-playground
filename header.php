		...
		tier.cost.forEach( function( cost, i ){
			...
			char.cost_affection.forEach( function( cost_affection ){    
				...
				if( cost_affection.type == cost.cost_type ){
					...
					if( /\*/.test( cost_affection.value ) ){
						
						cost.cost_value = parseFloat( cost.cost_value ) * parseFloat( cost_affection.value.replace(/[^0-9\.]/g,'' ) );
						
					}else if( /\\/.test( cost_affection.value ) ){
						
						cost.cost_value = parseFloat( cost.cost_value ) / parseFloat( cost_affection.value.replace(/[^0-9\.]/g,'' ) );

					}else{
					
						cost.cost_value = parseFloat( cost.cost_value ) + parseFloat( cost_affection.value.replace(/\s/g,'') );
			...
			if( parseInt( cost.cost_value ) > parseInt(char.pools[pool_id].cur && parseInt( cost.cost_value ) <= 100  ) ){ //Skip if cost is greater than 100
